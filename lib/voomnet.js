...
export function initVoomnet() {
  ...
  function notify(userId, texte, opts){
    ...
    /* son + bulle système, uniquement pour l'utilisateur connecté, jamais au démarrage */
    if (sessionUser && userId === sessionUser.id){
      const nv = o.niveau || 'info';
      if (sonPret) jouerSonNotif(nv);           /* le son, lui, attend la fin du démarrage */
      if (notifNavActivePour(nv)) notifierNavigateur(titreNotif(nv), texte, o.requestId || '');
    }
  }
  ...
  const VOL_CLE = 'voomnet_volume';
  const volumes = { faible: 0.6, moyen: 1, fort: 1.6 };
  function volumeSon(){ const v = storage.getItem(VOL_CLE); return volumes[v] || 1.6; }   /* « fort » par défaut */
  function setVolumeSon(v){ storage.setItem(VOL_CLE, volumes[v] ? v : 'fort'); }
  ...
  function contexteAudio(){
    try {
      ...
      if (audioCtx.state !== 'running' && audioCtx.resume){ try { audioCtx.resume(); } catch(e){} }
      return audioCtx;
    } catch(e){ return null; }
  }
  ...
  /* ---------- NOTIFICATIONS DU NAVIGATEUR (bulles système) ----------
     Elles s'affichent même si l'onglet VOOMNET est en arrière-plan, là où
     l'alarme sonore ne peut rien (le navigateur suspend l'audio). */
  const NOTIF_NAV_CLE = 'voomnet_notifnav';
  function notifNavSouhaitee(){ return storage.getItem(NOTIF_NAV_CLE) === '1'; }
  function notifNavDisponible(){
    return typeof window !== 'undefined' && typeof window.Notification !== 'undefined';
  }
  function notifNavEtat(){
    if (!notifNavDisponible()) return 'indisponible';
    const P = window.Notification.permission;
    if (P === 'granted') return notifNavSouhaitee() ? 'active' : 'autorisee';
    if (P === 'denied')  return 'refusee';
    return 'vierge';
  }
  const notifNavLabel = {
    active: 'activées 🔔', autorisee: 'autorisées mais désactivées',
    refusee: 'refusées par le navigateur', vierge: 'non activées',
    indisponible: 'non gérées par ce navigateur'
  };
  function activerNotifNav(){
    if (!notifNavDisponible()){ toast('Ce navigateur ne gère pas les notifications système.', 'warn'); return; }
    const demande = window.Notification.requestPermission();
    const suite = (p) => {
      if (p === 'granted'){
        storage.setItem(NOTIF_NAV_CLE, '1');
        notifierNavigateur('VOOMNET — notifications activées',
          'Vous serez prévenu même si l\'onglet est en arrière-plan.');
        toast('Notifications du navigateur activées 🔔', 'ok');
      } else {
        storage.setItem(NOTIF_NAV_CLE, '0');
        toast('Autorisation refusée — réactivez-la dans les réglages du navigateur (icône 🔒 à gauche de l\'adresse).', 'warn');
      }
      render();
    };
    if (demande && demande.then) demande.then(suite).catch(() => toast('Demande impossible', 'err'));
    else suite(window.Notification.permission);
  }
  function desactiverNotifNav(){
    storage.setItem(NOTIF_NAV_CLE, '0');
    toast('Notifications du navigateur désactivées', 'ok'); render();
  }
  function notifNavActivePour(niveau){
    if (!notifNavSouhaitee() || !notifNavDisponible()) return false;
    if (window.Notification.permission !== 'granted') return false;
    const cfg = alarmeCfg();
    if (cfg.niveaux === 'urgent') return niveau === 'urgent';
    if (cfg.niveaux === 'urgent_alerte') return niveau === 'urgent' || niveau === 'alerte';
    return true;
  }
  function notifierNavigateur(titre, corps, requestId){
    try {
      if (!notifNavDisponible() || window.Notification.permission !== 'granted') return;
      const n = new window.Notification(titre, { body: corps, icon: LOGO_URL(), tag: requestId || 'voomnet' });
      n.onclick = (e) => {
        try { window.focus(); } catch(err){ /* ignoré */ }
        if (requestId) go('detailDemande', { id: requestId, nav: 'mesDemandes' });
        n.close();
      };
    } catch(e){ /* ignoré */ }
  }
  const titreNotif = (nv) => 'VOOMNET — ' + (nv === 'urgent' ? '🔴 urgence' : nv === 'alerte' ? '🟠 alerte' : '🔵 information');
  ...
  /* Alerte VISUELLE de secours : le titre de l'onglet clignote tant qu'il reste
     des alertes non lues (utile si le son est coupé ou inaudible sur le poste). */
  const TITRE_NORMAL = 'VOOMNET — Gestion des Achats';
  let titreTimer = null, titreAlterne = false;
  function majAlerteVisuelle(nb){
    const badge = $('#bell-badge');
    if (badge) badge.classList.toggle('alarme', nb > 0);
    if (!nb){
      if (titreTimer){ clearInterval(titreTimer); titreTimer = null; }
      titreAlterne = false;
      try { document.title = TITRE_NORMAL; } catch(e){}
      return;
    }
    const peindre = () => {
      titreAlterne = !titreAlterne;
      try { document.title = titreAlterne ? `🔴 ${nb} ALERTE${nb > 1 ? 'S' : ''} — VOOMNET` : TITRE_NORMAL; } catch(e){}
    };
    peindre();
    if (!titreTimer) titreTimer = setInterval(peindre, 1200);
  }
  function jouerAcquittement(){
    sonPret = true;
    try { bipe(523, 0, .13, .26); bipe(784, .15, .24, .26); } catch(e){ /* ignoré */ }
  }
  ...
  function surveillerAlarme(){
    ...
    majAlerteVisuelle(nonLues.length);
  ...
  /* État réel du son (affiché dans ⚙️ Paramètres pour diagnostiquer) */
  function sonEtat(){
    try {
      const Ctx = (typeof window !== 'undefined') && (window.AudioContext || window.webkitAudioContext);
      if (audioCtx && audioCtx.state === 'running') return 'webaudio';
      if (!Ctx) return typeof window.Audio === 'function' ? 'secours' : 'indisponible';
      if (audioCtx && audioCtx.state === 'suspended') return 'bloque';
      return 'pret';
    } catch(e){ return 'indisponible'; }
  }
  const sonEtatLabel = () => ({
    webaudio: 'opérationnel (Web Audio)',
    pret: 'prêt — le son partira au premier clic',
    bloque: 'bloqué par le navigateur — cliquez sur 🔊 en haut à droite',
    secours: 'mode secours (Web Audio indisponible)',
    indisponible: 'audio non géré par ce navigateur'
  })[sonEtat()] || 'inconnu';

  /*Son de secours : on fabrique un WAV en mémoire (aucun fichier) et on le joue
     avec une balise <audio>. Cela contourne les navigateurs qui gardent le
     contexte Web Audio « suspendu » malgré le déblocage. */
  function bipeWav(freq, duree, volume){
    try {
      if (typeof window === 'undefined' || typeof window.Audio !== 'function') return false;
      const sr = 22050, n = Math.max(1, Math.floor(sr * duree));
      const buf = new ArrayBuffer(44 + n * 2), v = new DataView(buf);
      const texte = (o, t) => { for (let k = 0; k < t.length; k++) v.setUint8(o + k, t.charCodeAt(k)); };
      texte(0, 'RIFF'); v.setUint32(4, 36 + n * 2, true); texte(8, 'WAVEfmt ');
      v.setUint32(16, 16, true); v.setUint16(20, 1, true); v.setUint16(22, 1, true);
      v.setUint32(24, sr, true); v.setUint32(28, sr * 2, true); v.setUint16(32, 2, true); v.setUint16(34, 16, true);
      texte(36, 'data'); v.setUint32(40, n * 2, true);
      for (let k = 0; k < n; k++){
        const t = k / sr;
        const enveloppe = Math.min(1, t / 0.008) * Math.exp(-t * 3.2);
        const echant = Math.sin(2 * Math.PI * freq * t) * enveloppe * Math.max(0, Math.min(1, volume));
        v.setInt16(44 + k * 2, echant * 32767, true);
      }
      const octets = new Uint8Array(buf);
      let binaire = '';
      for (let k = 0; k < octets.length; k += 4096) binaire += String.fromCharCode.apply(null, octets.subarray(k, k + 4096));
      const a = new window.Audio('data:audio/wav;base64,' + btoa(binaire));
      a.volume = 1;
  ...
  function bipe(freq, decalage, duree, vol){
    const volume = Math.min(1, (vol || 0.2) * volumeSon());
    /* 1. Web Audio si le contexte tourne réellement */
    try {
      const ctx = contexteAudio();
      if (ctx && ctx.state === 'running'){
        const t0 = ctx.currentTime + decalage;
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = 'triangle';                       /* plus audible que la sinusoïde */
    ...
    } catch(e){ /* on bascule sur le secours */ }
    /* 2. Sinon : WAV généré à la volée */
    setTimeout(() => bipeWav(freq, duree, Math.min(1, volume * 3.2)), Math.max(0, decalage * 1000));
  }
  ...
  function jouerSonNotif(niveau, force){
    ...
    if (niveau === 'urgent')      { bipe(880, 0, .17, .32); bipe(988, .21, .17, .32); bipe(1175, .42, .22, .32); }
    else if (niveau === 'alerte') { bipe(660, 0, .15, .28); bipe(523, .19, .22, .28); }
    else                          { bipe(587, 0, .14, .22); }
  }
  ...
  /* ============================= 6ter. GRAPHIQUES (SVG pur, sans bibliothèque) ============================= */
  const COULEUR_STATUT = {
    brouillon: '#94a3b8', en_comparaison: '#64748b', en_attente: '#b45309',
    approuvee: '#16a34a', refusee: '#dc2626', modification: '#c2410c',
    commandee: '#000060', reception_partielle: '#6d28d9', cloturee: '#0f766e'
  };

  function grapheBarres(serie, opts){
    const o = Object.assign({ hauteur: 130, couleur: '#500070', couleurClaire: '#8b3fb8' }, opts || {});
    if (!serie || !serie.length) return '<div class="empty">Aucune donnée à afficher.</div>';
    const max = Math.max(1, ...serie.map((d) => +d.valeur || 0));
    const largeur = serie.length * 40, base = 108;
    const barres = serie.map((d, i) => {
      const h = Math.max(2, Math.round((+d.valeur || 0) / max * 96));
      const x = i * 40 + 7;
      return `<rect x="${x}" y="${base - h}" width="26" height="${h}" rx="4" fill="url(#vnDegrade)"><title>${esc(d.libelle)} : ${esc(d.texte || d.valeur)}</title></rect>`;
    }).join('');
    const axe = `<line x1="0" y1="${base}" x2="${largeur}" y2="${base}" stroke="#cbd5e1" stroke-width="1"/>`;
    return `<div class="graph" style="height:${o.hauteur}px">
      <svg viewBox="0 0 ${largeur} ${base + 12}" preserveAspectRatio="none" role="img" aria-label="Histogramme">
        <defs><linearGradient id="vnDegrade" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="${o.couleurClaire}"/><stop offset="100%" stop-color="${o.couleur}"/>
        </linearGradient></defs>
        ${axe}${barres}
      </svg>
      <div class="graph-labels">${serie.map((d) => `<span>${esc(d.libelle)}</span>`).join('')}</div>
    </div>`;
  }

  function grapheAnneau(serie){
    const total = (serie || []).reduce((t, p) => t + (+p.valeur || 0), 0);
    if (!total) return '<div class="empty">Aucune donnée à afficher.</div>';
    const R = 40, C = 2 * Math.PI * R;
    let decalage = 0;
    const morceaux = serie.filter((p) => +p.valeur > 0).map((p) => {
      const longueur = (+p.valeur / total) * C;
      const svg = `<circle r="${R}" cx="50" cy="50" fill="none" stroke="${p.couleur}" stroke-width="17"`
        + ` stroke-dasharray="${longueur} ${C - longueur}" stroke-dashoffset="${-decalage}" transform="rotate(-90 50 50)">`
        + `<title>${esc(p.libelle)} : ${p.valeur} (${Math.round(+p.valeur / total * 100)} %)</title></circle>`;
      decalage += longueur;
      return svg;
    }).join('');
    return `<div class="anneau-wrap">
      <svg class="anneau" viewBox="0 0 100 100" role="img" aria-label="Répartition">
        ${morceaux}<text x="50" y="47" text-anchor="middle" class="anneau-total">${total}</text>
        <text x="50" y="60" text-anchor="middle" class="anneau-legende">demandes</text>
      </svg>
      <ul class="legende">${serie.filter((p) => +p.valeur > 0).map((p) =>
        `<li><i style="background:${p.couleur}"></i><span>${esc(p.libelle)}</span><b>${p.valeur}</b></li>`).join('')}
      </ul>
    </div>`;
  }

  function achatsParMois(nbMois){
    const maintenant = new Date();
    const cle = (d) => d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0');
    const serie = [];
    for (let i = (nbMois || 12) - 1; i >= 0; i--){
      const d = new Date(maintenant.getFullYear(), maintenant.getMonth() - i, 1);
      serie.push({ cle: cle(d), libelle: d.toLocaleDateString('fr-FR', { month: 'short' }).replace('.', ''), valeur: 0 });
    }
    DB.orders.filter((o) => o.statut === 'Confirmée').forEach((o) => {
      const d = o.date ? new Date(o.date) : null;
      if (!d) return;
      const ligne = serie.find((x) => x.cle === cle(d));
      if (ligne) ligne.valeur += +o.total || 0;
    });
    return serie.map((x) => Object.assign(x, { texte: fmtM(x.valeur) }));
  }
  function repartitionStatuts(){
    return Object.keys(STATUTS).map((k) => ({
      cle: k, libelle: STATUTS[k].label, valeur: DB.requests.filter((r) => r.statut === k).length,
      couleur: COULEUR_STATUT[k] || '#94a3b8'
    }));
  }
  function topFournisseurs(nb){
    const cumul = {};
    DB.orders.filter((o) => o.statut === 'Confirmée').forEach((o) => {
      const k = suppName(o.supplierId);
      cumul[k] = (cumul[k] || 0) + (+o.total || 0);
    });
    return Object.entries(cumul).sort((a, b) => b[1] - a[1]).slice(0, nb || 5)
      .map(([nom, valeur]) => ({ libelle: nom, valeur, texte: fmtM(valeur) }));
  }
  ...
  function dashboardAdmin(){
    ...
    <div class="grid2">
    <div class="card"><h2>📈 Achats confirmés — 12 derniers mois</h2>${grapheBarres(achatsParMois(12))}</div>
    <div class="card"><h2>🍩 Répartition des demandes</h2>${grapheAnneau(repartitionStatuts())}</div>
  </div>
  <div class="card"><h2>🏅 Top 5 fournisseurs</h2>${grapheBarres(topFournisseurs(5), { couleur: '#0f766e', couleurClaire: '#5eead4' })}</div>
  ...
  function viewReports(){
    ...
    return `
    <div class="card"><h2>📈 Évolution des achats confirmés — 12 mois</h2>${grapheBarres(achatsParMois(12))}</div>
  <div class="stats-grid">
      ${statCard('💰','green', fmtM(totalAchats), 'Montant total des achats')}
    ...
    <div class="card"><h2>🔔 Notifications et sons</h2>
      <div class="form-grid">
        ...
        <div class="field"><label>Volume</label>
          <select data-volume="1">
            <option value="faible" ${volumeSon() === 0.6 ? 'selected' : ''}>Faible</option>
            <option value="moyen"  ${volumeSon() === 1 ? 'selected' : ''}>Moyen</option>
            <option value="fort"   ${volumeSon() === 1.6 ? 'selected' : ''}>Fort (recommandé)</option>
          </select>
          <span class="hint">Si vous n'entendez rien, choisissez « Fort » et cliquez sur « Tester les sons ».</span></div>
        <div class="field"><label>Essayer</label>
          <button class="btn outline" data-act="test-son">🔊 Tester les sons</button>
          <span class="hint">État du son : <b>${esc(sonEtatLabel())}</b>. Le son est fabriqué par le navigateur (aucun fichier) : vérifiez aussi le volume de votre poste.</span></div>
      </div>
      <div class="field mt"><label>Notifications du navigateur (bulles système)</label>
        <div class="actions-bar">
          ${notifNavEtat() === 'active'
            ? '<button class="btn outline sm" data-act="notifnav-off">🔕 Désactiver</button>'
            : '<button class="btn primary sm" data-act="notifnav-on">🔔 Activer les notifications</button>'}
          ${notifNavEtat() === 'active' ? '<button class="btn outline sm" data-act="test-notifnav">Tester une bulle</button>' : ''}
        </div>
        <span class="hint">État : <b>${esc(notifNavLabel[notifNavEtat()] || '')}</b>. Une fois activées, les alertes s'affichent <b>même si l'onglet VOOMNET est en arrière-plan</b> (le navigateur joue alors son propre son).</span>
      </div>
    </div>
  ...
  document.addEventListener('click', e => {
    ...
    switch (act) {
      ...
      case 'notifnav-on':     activerNotifNav(); break;
      case 'notifnav-off':    desactiverNotifNav(); break;
      case 'test-notifnav':   notifierNavigateur('VOOMNET — test', 'Les notifications système fonctionnent correctement 🔔'); break;
      case 'toggle-son': {
  ...
  document.addEventListener('change', e => {
    ...
    if (t.dataset && t.dataset.volume){
      setVolumeSon(t.value); sonPret = true; jouerSonNotif('alerte', true);
      toast('Volume réglé 🔊', 'ok'); render(); return;
    }
