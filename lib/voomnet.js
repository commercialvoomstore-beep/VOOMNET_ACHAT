...
export function initVoomnet() {
  ...
  function saveDB(){ storage.setItem(DB_KEY, JSON.stringify(DB)); if (remote) remote.sync(DB); }
  function loadDB(){
  ...
  async function exportPdfRows(titre, rows, nomFichier){
    ...
    doc.setFillColor(11, 31, 58); doc.rect(0, 0, W, 58, 'F');
  ...
  function applyRemote(db){
    ...
    if (state.page === 'nouvelleDemande' || $('#modal-root').classList.contains('open')){ pendingRemote = db; return; }
