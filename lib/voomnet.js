...
export function initVoomnet() {
  ...
  function saveDB(){ storage.setItem(DB_KEY, JSON.stringify(DB)); }
  function loadDB(){
  ...
  function seedData(){
    ...
    return { users, suppliers, requests, orders, receptions, notifications, meta:{ achCounter:5, bcCounter:2, seuilOffres:3, tva:0, poids:{prix:50, delai:20, garantie:20, paiement:10} } };
  }
  ...
  function suppName(id){ const s = byId(DB.suppliers, id); return s ? s.nom : 'Fournisseur supprimé'; }
  ...
  function seuilOffres(){ const n = +meta().seuilOffres; return n > 0 ? n : 3; }
  ...
  function negotiationHTML(r){
    ...
    return `
    ${negoToolbarHTML(r)}
    <div class="chosen-banner" style="background:var(--blue-bg);border-color:var(--primary);color:#1e3a8a">📞 Négociez <b>article par article</b> auprès de chaque fournisseur : saisissez le prix unitaire obtenu — le total de la ligne, le coût total rendu (remise, TVA, livraison) et le <b>score multicritère</b> se calculent automatiquement.</div>
    ${banner}
    ${negoPoidsHTML()}
  ...
  /* ---- Notifications ---- */
  function notify(userId, texte){ DB.notifications.unshift({id:uid('n'), userId, texte, date:nowISO(), lu:false}); }
  ...
  function renderNotifPanel(){
    ...
    $('#notif-panel').innerHTML =
      `<div class="notif-head">🔔 Notifications<button data-act="notif-readall">Tout marquer comme lu</button></div>
       <div class="notif-list">${list.length ? list.map(n =>
          `<div class="notif-item ${n.lu?'':'unread'}"><div>${esc(n.texte)}<time>${fmtDT(n.date)}</time></div></div>`
  ...
  function recentRequestsTable(rows, nav){
    ...
    return `<div class="table-wrap"><table class="tbl">
      ...
      ${rows.map(r => `<tr>
        ...
        <td class="num">${fmtM(montantDemande(r))}</td>
  ...
  function viewDashboard(){
    ...
    if (role === 'responsable') return dashboardResponsable();
  ...
  function viewRequestsList(page){
    ...
    let html = `<div class="card"><h2>${title}
        <span class="h-actions">${page === 'mesDemandes' ? '<button class="btn primary sm" data-act="nav" data-page="nouvelleDemande">＋ Nouvelle demande</button>' : ''}<button class="btn outline sm" data-act="export-requests">📊 Exporter Excel</button></span>
      </h2>
      <div class="toolbar">
  ...
  function collectStep1(silent){
    ...
    if (!silent){
      ...
      if (draft.articles.some(a => !a.designation.trim() || +a.quantite < 1 || +a.prix < 1)) {
        toast('Chaque article doit avoir une désignation, une quantité ≥ 1 et un coût présumé.', 'warn'); return false;
      }
    }
    return true;
  }
  ...
  function wizSupplierList(){
    ...
    requestAnimationFrame(() => { const el = $('#selcount'); if (el) el.innerHTML = `<b>${selN}</b> fournisseur(s) sélectionné(s) — la procédure exige au moins <b>3</b> fournisseurs.`; });
  ...
  function viewOrders(){
    ...
    return `<div class="card"><h2>📦 ${isDem?'Mes commandes':'Commandes'}<span class="h-actions"><button class="btn outline sm" data-act="export-orders">📊 Exporter Excel</button></span></h2>
  ...
  function viewReceptions(){
    ...
    return `<div class="card"><h2>🚚 ${isDem?'Mes réceptions':'Réceptions'}</h2>
    <div class="table-wrap"><table class="tbl">
      ...
      ${recs.map(rc => {
        ...
        return `<tr>
          ...
          <td>${fmtD(rc.date)}</td>
          <td>${st}</td>
          <td>${canRec?`<button class="btn sm primary" data-act="open-reception" data-id="${o.id}">${rc.statut==='Partielle'?'Compléter':'Enregistrer'}</button>`:''}</td>
        </tr>`;
      }).join('')}
    </table></div></div>`;
  }
  ...
  function printOrder(orderId){
    ...
    $('#print-root').innerHTML = `
      <div style="text-align:center;border-bottom:3px solid #2563eb;padding-bottom:12px;margin-bottom:16px">
        <div style="font-size:24px;font-weight:800;color:#0b1f3a">VOOMNET TECHNOLOGY</div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12.5px">
        <tr style="background:#eef2f7"><th style="border:1px solid #cbd5e1;padding:6px;text-align:left">Produit</th><th style="border:1px solid #cbd5e1;padding:6px">Qté</th><th style="border:1px solid #cbd5e1;padding:6px">Prix unitaire</th><th style="border:1px solid #cbd5e1;padding:6px">Total</th></tr>
        ${o.lignes.map(l=>`<tr><td style="border:1px solid #cbd5e1;padding:6px">${esc(l.designation)}</td><td style="border:1px solid #cbd5e1;padding:6px;text-align:center">${l.quantite}</td><td style="border:1px solid #cbd5e1;padding:6px;text-align:right">${fmtM(l.prix)}</td><td style="border:1px solid #cbd5e1;padding:6px;text-align:right">${fmtM(l.quantite*l.prix)}</td></tr>`).join('')}
        <tr><td colspan="3" style="border:1px solid #cbd5e1;padding:6px;text-align:right"><b>MONTANT TOTAL</b></td><td style="border:1px solid #cbd5e1;padding:6px;text-align:right"><b>${fmtM(o.total)}</b></td></tr>
      </table>
  ...
  function showImportPreview(rows){
    ...
    $('#import-preview').innerHTML = `
      <div class="chosen-banner" style="background:var(--blue-bg);border-color:var(--primary);color:#1e3a8a">📄 Aperçu du fichier — <b>${valid.length} fournisseur${valid.length > 1 ? 's' : ''} détecté${valid.length > 1 ? 's' : ''}</b></div>
  ...
  function viewSettings(){
    ...
    <div class="card"><h2>⚙️ Paramètres</h2>
      ...
      <div class="kv"><span>Stockage des données</span><span>localStorage du navigateur — ${kb} Ko</span></div>
    ...
    <div class="card"><h2>⚖️ Comparaison des prix (étape 3)</h2>
      <div class="form-grid">
        <div class="field"><label>Nombre minimum d'offres complètes</label>
          ...
          <span class="hint">Un fournisseur n'ayant pas chiffré tous les articles est marqué « ⚠️ SANS RÉPONSE » et exclu de la comparaison.</span></div>
  ...
  document.addEventListener('click', e => {
    ...
    switch (act) {
      ...
      case 'notif-readall': DB.notifications.forEach(n => { if (n.userId === sessionUser.id) n.lu = true; }); saveDB(); renderNotifPanel(); updateBell(); break;
      ...
      case 'wiz-next': {
        ...
        if (step === 1){ if (!collectStep1()) return; draft.step = 2; }
          ...
          if (draft.supplierIds.length < 3){ toast('Sélectionnez au moins 3 fournisseurs pour pouvoir comparer les offres.', 'warn'); return; }
          pushHist(draft, draft.supplierIds.length + ' fournisseurs sélectionnés');
          draft.step = 3;
        }
        else if (step === 3){
          const complets = offresCompletes(draft);
          const seuil = seuilOffres();
          ...
          if (!draft.chosenSupplierId){ toast('Sélectionnez d\'abord le fournisseur retenu.', 'warn'); return; }
          if (!draft.justification){ toast('Renseignez la justification du choix.', 'warn'); return; }
  ...
  document.addEventListener('change', e => {
    ...
    if (t.dataset && t.dataset.meta){
      ...
      DB.meta = DB.meta || {};
      DB.meta[k] = (k === 'seuilOffres' || k === 'tva') ? Math.max(0, Math.min(100, +t.value || 0)) : t.value;
