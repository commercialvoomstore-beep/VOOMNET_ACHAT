...
export function initVoomnet() {
  ...
  function negotiationHTML(r){
    ...
    return `
    ${negoToolbarHTML(r)}
    <div class="chosen-banner" style="background:#ecfdf5;border-color:var(--primary);color:#065f46">📞 Négociez <b>article par article</b> auprès de chaque fournisseur : saisissez le prix unitaire obtenu — le total de la ligne, le coût total rendu (remise, TVA, livraison) et le <b>score multicritère</b> se calculent automatiquement.</div>
    ${banner}
    ${negoPoidsHTML()}
  ...
  function showImportPreview(rows){
    ...
    $('#import-preview').innerHTML = `
      <div class="chosen-banner" style="background:#ecfdf5;border-color:var(--primary);color:#065f46">📄 Aperçu du fichier — <b>${valid.length} fournisseur${valid.length > 1 ? 's' : ''} détecté${valid.length > 1 ? 's' : ''}</b></div>
